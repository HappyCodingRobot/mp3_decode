...
#define PANEL1_PANEL_TYPE SSD1306_NONE
...
bool ICACHE_FLASH_ATTR ssd1306_init(uint8_t id)
{
    ...
    ctx = (oled_i2c_ctx *) zalloc(sizeof(oled_i2c_ctx));
    if (ctx == NULL)
    {
    ...
    if (id == 0)
    {
        ...
        ctx->buffer = (uint8_t*) zalloc(1024); // 128 * 64 / 8
        ctx->width = 128;
        ctx->height = 64;
  #elif (PANEL0_TYPE == SSD1306_128x32)
        ctx->type = SSD1306_128x32;
        ctx->buffer = (uint8_t*) zalloc(512);  // 128 * 32 / 8
        ctx->width = 128;
        ctx->height = 32;
  #else
